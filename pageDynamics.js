...
function addPartnersToList(currentPartnerListIndex, currentAllCombosList, mode) {
    for (var i = 0; i < currentAllCombosList[currentPartnerListIndex].length; i++) {
        //<li class="collection-item">Fred and Eric</li>
        if (currentAllCombosList[currentPartnerListIndex][i][0] != "Nobody" && currentAllCombosList[currentPartnerListIndex][i][1] != "Nobody") {
            $(".partnerList").append('<li class="collection-item">' + currentAllCombosList[currentPartnerListIndex][i][0] + ' and ' + currentAllCombosList[currentPartnerListIndex][i][1] + '</li>');
        } else if (currentAllCombosList[currentPartnerListIndex][i][0] === "Nobody") {
            $(".partnerList").append('<li class="collection-item">' + currentAllCombosList[currentPartnerListIndex][i][1] + ' join a partnership</li>');
        } else {
            $(".partnerList").append('<li class="collection-item">' + currentAllCombosList[currentPartnerListIndex][i][0] + ' join a partnership</li>');
        }
    }
}

function showPartnerListProgressOnHeader(currentPartnerListIndex, currentAllCombosListLength, currentPartnershipName) {
    if (currentPartnershipName != null) {
        $(".partnersTitle").text(currentPartnershipName + " (" + (currentPartnerListIndex + 1) + " of " + (currentAllCombosListLength) + ")");
    } else {
        $(".partnersTitle").text("Partners (" + (currentPartnerListIndex + 1) + " of " + (currentAllCombosListLength) + ")");
    }
}
...
function addPerson(name) {
    ...
    function isNameAlreadyUsed(name) {
        ...
        $('.peopleList li').each(function(){
            ...
            var nameToPush = $(this).text().slice(0, $(this).text().length - 5);
    ...
    if (isNameAlreadyUsed(name) === true) {
        ...
        if(name != "") {
            $(".peopleList").prepend('<li class="collection-item">' + name + '<a style="float: right"><i class="material-icons deletePerson clickableIcon">close</i></a></li>');
...
function savePartnership(name, currentPartnerListIndex, currentAllCombosList) {
    localStorage.setItem("PartnerCreator: " + name, JSON.stringify([currentAllCombosList, currentPartnerListIndex]));
}
...
$(document).ready(function() {
    ...
    */
    ...
    $(".retrieveSaveModalButton").click(function(e) {
        ...
        for ( var i = 0, len = localStorage.length; i < len; ++i ) {
            if (localStorage.key(i).slice(0, 15) === "PartnerCreator:") {
                $(".saveFileList").prepend('<li class="collection-item"><i class="material-icons deleteSaveFileButton clickableIcon" style="float: left">delete</i>' + localStorage.key(i).slice(16, localStorage.key(i).length) + '<a style="float: right"><i class="material-icons loadSaveFileButton clickableIcon">arrow_forward</i></a></li>');
            }
        }
    ...
    $('.saveFileList').on('click', 'i', function() {
        ...
        if ($(this).parent().text() === "arrow_forward") {
            //we slice it because it add arrow_forward to the end of the text
            var name = ($(this).parent().parent().text()).slice(0, -13);
            name = name.slice(6, name.length);

            currentAllCombosList = JSON.parse(localStorage.getItem("PartnerCreator: " + name))[0];
            currentPartnerListIndex = JSON.parse(localStorage.getItem("PartnerCreator: " + name))[1];
            ...
            showPartnerListProgressOnHeader(currentPartnerListIndex, currentAllCombosList.length, currentPartnershipName);
            ...
            var name = ($(this).parent().text()).slice(0, -13);
            name = name.slice(6, name.length);
    ...
    $(".sendPeople").click(function(e) {
        var people = []
        $('.peopleList li').each(function(){
            people.push($(this).text().slice(0, $(this).text().length - 5));
        });
    ...
    $(".fullscreenButton").click(function(e) {
        ...
        for (var i = 0; i < currentAllCombosList[currentPartnerListIndex].length; i++) {
            if (currentAllCombosList[currentPartnerListIndex][i][0] != "Nobody" && currentAllCombosList[currentPartnerListIndex][i][1] != "Nobody") {
                $(".fullscreenPartnerListHolder div:nth-child(" + columnNumber + ") ul").append('<li class="collection-item">' + currentAllCombosList[currentPartnerListIndex][i][0] + ' and ' + currentAllCombosList[currentPartnerListIndex][i][1] + '</li>');
            } else if (currentAllCombosList[currentPartnerListIndex][i][0] === "Nobody") {
                $(".fullscreenPartnerListHolder div:nth-child(" + columnNumber + ") ul").append('<li class="collection-item">' + currentAllCombosList[currentPartnerListIndex][i][1] + ' join a partnership</li>');
            } else {
                $(".fullscreenPartnerListHolder div:nth-child(" + columnNumber + ") ul").append('<li class="collection-item">' + currentAllCombosList[currentPartnerListIndex][i][0] + ' join a partnership</li>');
            }
            columnNumber += 1;
            if (columnNumber == 5) {
                columnNumber = 1;
